...
const conteinerCheckboxIndex = document.querySelector(
  "#contenedor-input-index"
);
const conteinerCheckboxFuture = document.querySelector(
  "#contenedor-checkbox-future"
);
const conteinerCheckboxPast = document.querySelector(
  "#contenedor-checkbox-past"
);
...
function printCards(array, node, rutaID, rutaImg) {
  ...
  if (array.length !== 0) {
    array.forEach((event) => {
      node.innerHTML += `<div class="card col-4" style="width: 18rem;">
...
function printDetail(evento, node, data) {
  ...
  node.innerHTML = `<div class="col-md-6 text-center">
                        <img src="${
                          evento.image
                        }" alt="fotografia evento" class="img-fluid rounded-start">
                    </div>
                        ...
                        <div class="card-body text-center">
                            ...
                            <div class="text-start ps-5 row">
                                <div class="col-md-7 mb-3">
                                    <p class="card-text"><span class="datos-detail">Date:</span> ${
                                      evento.date
                                    }</p>
                                    <p class="card-text"><span class="datos-detail">Place:</span> ${
                                      evento.place
                                    }</p>
                                    <p class="card-text"><span class="datos-detail">Capacity:</span> ${
                                      evento.capacity
                                    }</p>
                                    <p class="card-text">${assistanceOrEstimate(
                                      data,
                                      evento
                                    )}</p>
                                    <p class="card-text"><span class="datos-detail">Price:</span> u$s ${
                                      evento.price
                                    }</p>
                                </div>
...
function categoryReturn(array) {
  ...
  return category.filter(
    (elemento, index) => category.indexOf(elemento) === index
  );
}
...
function printCheckboxs(array, node) {
  array.forEach((event) => {
    node.innerHTML += `<label class="pe-3"><input type="checkbox" class="checkbox">${event}</label>`;
  });
}
...
function filtradora(
  ...
  if (arrayCheckeado.length > 0 && textInput !== "") {
    arrayCheckeado.map((categoria) => {
      arrayResultado.push(
        ...arrayEventos.filter(
          (event) =>
            (event.name.toLowerCase().includes(normalizeString(textInput)) ||
              event.category
                .toLowerCase()
                .includes(normalizeString(textInput))) &&
  ...
  } else if (arrayCheckeado.length > 0 && textInput === "") {
    // OK

    arrayCheckeado.map((categoria) => {
      arrayResultado.push(
        ...arrayEventos.filter((event) => event.category === categoria)
      );
    ...
    arrayResultado.push(
      ...arrayEventos.filter(
        (event) =>
          event.name.toLowerCase().includes(normalizeString(textInput)) ||
          event.category.toLowerCase().includes(normalizeString(textInput))
      )
    );
  } else {
...
function calcCategory(arrayEvents) {
  ...
  arrayCategorys.forEach((categoria) => {
    arrayResultado.push(
      arrayEvents.filter((evento) => evento.category === categoria)
    );
  });
...
function printTableEvents(arrayDatos, contenedorTable) {
  ...
  arrayDatos.forEach((evento) =>
    evento.capacity
      ...
      : resultado.push(
          `${evento.name} with ${evento.percentage}% of assistance`
        )
  );
...
function calcTableCategory(arrayCategorias) {
  ...
  const assitOrEstimate = arrayCategorias.reduce(
    (total, evento) =>
      evento.assistance
        ? total + Number(evento.assistance)
        : total + Number(evento.estimate),
    0
  );

  const capacityTotal = arrayCategorias.reduce(
    (total, evento) => total + Number(evento.capacity),
    0
  );

  const percentageTotal = (assitOrEstimate * 100) / capacityTotal;

  const priceTotal = arrayCategorias.reduce(
    (total, evento) =>
      evento.assistance
  ...
  const priceFormat = new Intl.NumberFormat().format(
    (priceTotal / arrayCategorias.length).toFixed(2)
  );
...
async function getFetch() {
  ...
  await fetch(APIurl)
    ...
    .then((data) => {
      ...
      const checkboxsIndex = document.querySelectorAll(
        'input[type="checkbox"]'
      );
      ...
      checkboxsIndex.forEach((elemento) =>
        elemento.addEventListener("click", (e) => {
          // evento click en los checkbox y filtrado
          ...
          if (e.target.checked) {
            ...
            arrayCheckedIndex = arrayCheckedIndex.filter(
              (elemento) => elemento !== e.target.parentNode.textContent
            );
            filtradora(
              arrayCheckedIndex,
              eventos,
      ...
      inputIndex.addEventListener("keyup", (e) => {
        // evento keyup en el search y filtrado
